...
# Importing dataset
psql -U ituser -d postgres -f company_db.sql

# Executing the 3 queries

echo "task 1"
psql -U ituser -d postgres -c "select count(*) as total_number_employees from employees;" > /var/log/db_setup.log

echo "task 2"
echo "Enter department name:"
read department_name

psql -U ituser -d postgres -c "
